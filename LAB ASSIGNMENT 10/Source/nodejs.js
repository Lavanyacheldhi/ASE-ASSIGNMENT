...
var app = express();
var result={'body': []};
var url = 'mongodb://lavanya2395:Lavanya@408@ds151820.mlab.com:51820/lavanya';
app.use(cors());
...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function(db, data, callback) {
        db.collection('lavanya23').insertOne( data, function(err, result) {
            if(err)           {
...
app.post('/get-data',function (req,res) {
    ...
    var findUser = function(db, callback) {
        var cursor =db.collection('lavanya23').find();
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('lavanya23').updateOne({"_id":ObjectId(id)},{$set:item}, function(err, result) {
            if(err)            {
                res.write("Registration Failed");
                res.end();            }
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        if(err)        {
            res.write("Registration Failed");
            res.end();        }
        db.collection('lavanya23').deleteOne({"_id": ObjectId(id)}, function(err, result) {
